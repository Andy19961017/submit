...
class AWACAgent(DQNAgent):
    ...
    def get_qvals(self, critic, obs, action):
        ...
        q_value = torch.gather(qa_values, 1, action.long().unsqueeze(1)).squeeze(1)
        # Q: what about continuous case?
        # Q: single or multiple?
        assert q_value.shape == (obs.shape[0],)
        return q_value
    ...
    def estimate_advantage(self, ob_no, ac_na, re_n, next_ob_no, terminal_n, n_actions=10):
        ...
        v_pi = dist.log_prob(ac_na).exp() * q_values
        advantages = q_values - v_pi
    ...
    def train(self, ob_no, ac_na, re_n, next_ob_no, terminal_n):
        ...
        if (self.t > self.learning_starts and self.t % self.learning_freq == 0 and self.replay_buffer.can_sample(self.batch_size)):
            ...
            expl_bonus = (expl_bonus - np.mean (expl_bonus)) / (np.std (expl_bonus) + 0.000001)
            ...
            adv = self.estimate_advantage(ob_no, ac_na, mixed_reward, next_ob_no, terminal_n)
            # Q: is mixed_reward the right reward?
            actor_loss = self.awac_actor.update(ob_no, ac_na, adv_n=adv)
            # Q: what's the difference between self.eval_policy and self.awac_actor?
    ...
    def step_env(self):
        ...
        # Q: how can offline RL interact with env?
